<?php
// ── page/test_plans.php ─────────────────────────────
// ADMIN VERSION: Full CRUD, ALL projects/clients accessible
// TP ID is project-specific: each project starts at TP-001
// Same UI/logic as u_test_plans.php but admin has unrestricted access
session_start();
...
// ════════════════════════════════════════════════════════════
//  FETCH DATA — ADMIN: ALL clients, ALL projects
// ════════════════════════════════════════════════════════════
$clients_list = [];
...
// Build projects JSON for modal dropdown (admin: ALL projects)
$modal_projects_json = json_encode($projects_list);
...
<script>
// ═══════════════════════════════════════════════════════
//  CONFIG — Admin has access to ALL projects
// ═══════════════════════════════════════════════════════
const allProjects = <?= $all_projects_json ?>;
const modalProjects = <?= $modal_projects_json ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (params.has('added') || params.has('updated') || params.has('deleted') || params.has('imported') || params.has('add_err') || params.has('edit_err') || params.has('import_err')) {
        setTimeout(function() {
    ...
    // Populate import project dropdown with ALL projects
...
function openEditModal(tp) {
    ...
    document.querySelectorAll('#planForm input[name="technologies[]"]').forEach(cb => {
        cb.checked = predefinedTechs.includes(cb.value) && techVals.includes(cb.value);
    });
    techVals.forEach(tech => {
        if (!predefinedTechs.includes(tech)) {
...
document.getElementById('confirmDelBtn').addEventListener('click', function() {
    if (delId) {
        fetch('?delete=' + delId + '&ajax=1', {method: 'GET'})
            ...
            .then(d => {
                if (d.success) {
                    const row = document.querySelector('tr td button[onclick*="' + delId + '"]');
...
function onFileSelect(input) {
    ...
    const label = document.getElementById('csvDropText');
    const zone  = document.getElementById('csvDropZone');
    if (file) {
        ...
        label.textContent = 'Drag & drop CSV here or click to browse';
